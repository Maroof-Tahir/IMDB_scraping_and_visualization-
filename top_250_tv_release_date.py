...
def scrape_imdb_top_250():
    url = 'https://www.imdb.com/chart/toptv/?ref_=nv_tvv_250&sort=release_date%2Cdesc'
    ...
    for movie in movies:
        ...
        creator = "N/A"
        try:
            creator = movie.find_element(By.XPATH, './/span[contains(@class, "sc-59c7dc1-5")]/a[contains(@class, "ipc-link ipc-link--base dli-creator-item")]').text.strip()
        except:
            pass
...
def insert_into_database(movies):
    ...
    cursor.execute(''' 
    IF NOT EXISTS (SELECT * FROM sysobjects WHERE name='IMDB_Top_250_Shows_release_date' AND xtype='U')
    BEGIN
        CREATE TABLE IMDB_Top_250_Shows_release_date (
    ...
    for movie in movies:
        cursor.execute('''
            INSERT INTO IMDB_Top_250_Shows_release_date 
...
if __name__ == '__main__':
    ...
    if movies:
        print(f"Scraped Top {len(movies)} TvShows successfully!")
        
        # Insert the scraped data into SQL Server
        insert_into_database(movies)
        print("Data Imported to SQL table successfully")
    else:
